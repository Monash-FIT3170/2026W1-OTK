# FIT3170 Handover Document v1

**Group 6 — Tuesday 11am - 2pm**

Eric Blyth (32529082), Gia Thinh Nguyen (33721661)

## Version History

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 1/9/2026 | v1.0 | Submission for Milestone 3 | Gia Thinh (SA), Eric (SA) |

## Table of Contents

- [Infrastructure & Architecture](#infrastructure--architecture)
  - [Frameworks and Plugins Used](#frameworks-and-plugins-used)
...
- [Adding Game Content](#adding-game-content)
  ...
  - [Adding Background Scenes](#adding-background-scenes)
...
- **TailwindCSS** — a utility-first CSS framework that is used for the styling of the project.
- **Motion** — an animation library for React that is used for all animations in the project.
- **Bcrypt** — a hashing library used to encrypt user passwords.
...
https://2026w1-otk-production.up.railway.app/
...
- Use one method per file.
- Use one publication per file.
- When adding a file, import it into the nearest `index.js`.
- Create a new folder under `/imports/api/` when adding a new feature.
- Keep unit tests in the same folder as the component or feature they test.
- Name unit test files using the original filename followed by `.tests`.
- Ensure new tests are imported through `tests/main.js`.
...
It is recommended to download Mongo Shell, or MongoSH, to be able to see the contents of the database using the command line.

Once installed, ensure that the project is running and run the following command in another terminal window:

```
mongosh mongodb://127.0.0.1:3001/meteor
```

This will open the mongosh terminal which allows for checking of the database contents.

To see all collections run the following command:

```
show collections
```

To see the contents of a specific collection run the following command:
...
1. Download and install NodeJS, the latest LTS version is recommended.
2. Verify the installation by running `node --version` and `npm --version` in a terminal window. Both these commands should print a version number.
3. Download and install MeteorJS by running the following command in a terminal window: `npx meteor`. This should install Meteor CLI globally. Verify the installation by running `meteor --version`. This should print a version number.
4. Clone the code repository locally by running the following command:
   ```
   git clone https://github.com/Monash-FIT3170/2026W1-OTK.git <filePath>
   ```
   Where `<filePath>` is the path to the folder where the project will be cloned to.
5. Open a terminal window in the folder where the project was cloned.
...
> **Note:** The Dockerfile and Docker Compose configuration should not be modified unnecessarily, as changes to these files can affect the production deployment.
...
Any new files should follow the project's existing naming and organisation conventions. If the new content requires an API feature, follow the API structure described in the [Codebase Structure](#codebase-structure) section.
