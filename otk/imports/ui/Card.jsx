...
export default function Card({cardProps}) {
  const costFontColour = cardProps.currentCost != cardProps.baseCost ? "text-lime-700" : "text-blue-700";
  const attackFontColour = cardProps.currentAttack != cardProps.baseAttack ? "text-lime-700" : "text-red-700";

  return (
    <motion.div >
      <motion.div whileHover={{scale: 1.1, backgroundColor: "oklch(90.5% 0.233 277.117)"}} 
      className="flex flex-col gap-2 bg-slate-400 rounded-xl shadow-md p-2 aspect-5/7 min-w-40 min-h-50 max-h-1/7 max-w-1/8 box-border border-slate-600 border-1">
        <div className="flex flex-2/12 min-h-5 justify-center box-border border-1 bg-slate-300 box-border border-slate-600 border-1 font-mono 
        ...
        <div className="flex flex-4/10 flex-row gap-3 grow-0">
          ...
          <div className="flex flex-1/6 box-border border-slate-600 border-1 aspect-5/4"> 
            <img src="https://assetsio.gnwcdn.com/pokemon-tcg-25th-anniversary-pikachu-base-set-artwork.png?width=690&quality=85&format=jpg&dpr=3&auto=webp"/>
            {/* TODO - Placeholder image */}
          </div>
        </div>
        <div className="flex flex-7/10 min-h-20 box-border border-1 bg-slate-300 box-border border-slate-600 border-1 font-mono 
