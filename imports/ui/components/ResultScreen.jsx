...
export function ResultScreen({ result, enemyName }) {
  const isStageWin = result === 'stageWin';
  const isWin = result === 'win' || isStageWin;
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center gap-6">
      {isWin ? (
        <>
          <h1 className="text-5xl font-bold text-yellow-400">
            {isStageWin ? 'Stage Clear!' : 'Victory!'}
          </h1>
      ...
      <button
        ...
        onClick={() =>
          Meteor.call(isStageWin ? 'game.continueStage' : 'game.newGame')
        }
      >
        {isStageWin ? 'Continue' : isWin ? 'Play Again' : 'Try Again'}
      </button>
      <button
